...
def plot_spectra_phot_1panel(spec, phot):
    ...
    for (spec_wave_um, spec_flux_cgs, spec_err_cgs, spec_name) in zip(spec_waves_um, spec_fluxes_cgs, spec_errs_cgs, spec_names):
        ax.step(spec_wave_um, spec_flux_cgs, color="k", label=spec_name, where="mid")
...
def plot_spectra_phot_multipanel(spec, phot):
    ...
    fig, axes = plt.subplots(nrows=n_spec, ncols=1, figsize=(12, 5*n_spec))

    # Colors
    cmap = cm.get_cmap("rainbow")
    colors = [cmap(i/len(phot_waveffs_um)) for i in range(len(phot_waveffs_um))]

    for i, (spec_wave_um, spec_flux_cgs, spec_err_cgs, spec_name) in enumerate(zip(spec_waves_um, spec_fluxes_cgs, spec_errs_cgs, spec_names)):
        ax = axes[i]
...
def main():
    ...
    obs_names = [obs_kwargs[k]["prefix"] for k in obs_kwargs.keys()]
    spec_names = obs_names.copy()
    spec_names.remove("phot")
    ...
    phot_mask = np.ones_like(phot_filters)
    ...
    for i, obs_key in enumerate(obs_kwargs.keys()):
        ...
        # Load data
        # -- photometry (skip)
        if obs_kwargs[obs_key]["prefix"] == "phot":
            continue
        # -- prism
        elif obs_kwargs[obs_key]["prefix"] == "prism":
    ...
    plot_spectra_phot_multipanel(spec, phot)
