...
def plot_linked_parameters(sim_tree, ind_trees, filters, linked_params, wv_to_save, waveffs, plot_priors=False):
    ...
    for i, linked_param in enumerate(linked_params):
        ...
        # Optionally, show prior ranges
        if plot_priors:
            for prior_type, prior_type_dict in prior_dict.items():
                # -- uniform priors
                if prior_type == "uniform":
                    for param, range in prior_type_dict.items():
                        if param == linked_param:
                            lo, hi = range
                            ax.axhline(lo, ls="--", color="k")
                            ax.axhline(hi, ls="--", color="k")
...
# -- sim fit
# out_name = "sersic_mcmc_simfit_poly2_results.asdf"
# out_name = "sersic_mcmc_simfit_poly3_results.asdf"
# out_name = "sersic_mcmc_simfit_bspline4_results.asdf"
# out_name = "sersic_mcmc_simfit_bspline5_results.asdf"
out_name = "sersic_mcmc_simfit_bspline5_results_tightpriors.asdf"
...
# -- linked parameters
fig = plot_linked_parameters(sim_tree=sim_tree, ind_trees=ind_trees, filters=filters, linked_params=linked_params, wv_to_save=wv_to_save, waveffs=waveffs, plot_priors=True)
